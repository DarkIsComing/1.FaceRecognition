...
def delete_same_img(args, err_msg, id_imgspath):
    id_imgsdir = id_imgspath[0].replace('\\', '/').split('/')[0]
    #print(id_imgsdir)
    #print(err_msg)
    for key in err_msg:
...
def write_worker(q_out, args):
    ...
    while more:
        ...
        # 如果为空，说明已经预测完了所有图片
            ...
            if args.delete_same == True:
                delete_same_img(args, err_msg, id_imgspath)
            elif (err_msg is not None) and (args.delete_same ==False):
...
def parse_args():
    ...
    parser.add_argument('--outdir',
                        default='F:/4.image_clean',
                        help='path to folder containing images.')

    # 预训练模型的路径
    parser.add_argument('--model', default='../models/model-y1-test2/model,0', help='path to load model.')

    # 判定是否为同一人的相似度阈值，上次记录0.89
    parser.add_argument('--threshold', default=0.42, type=float, help='')

    # 删除相同的照片，只保留一张，在这之前，需要想找到能够判断是否为相同照片的阈值
    parser.add_argument('--delete_same', default=True,  type=bool, help='')
