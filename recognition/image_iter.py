...
class FaceImageIter(io.DataIter):
    ...
    def __init__(self, batch_size, data_shape,
                 ...
                 rand_mirror = True, cutoff = 0, color_jittering = 0,
                 images_filter = 0,
                 data_name='data', label_name='softmax_label', **kwargs):

        """

        :param batch_size: 每次迭代batch_size
        :param data_shape: 输出的形状
        :param path_imgrec: rec文件的路径
        :param shuffle: 是否进行洗牌，随机打乱
        :param aug_list:
        :param mean:
        :param rand_mirror: # 随机镜像翻转
        :param cutoff:
        :param color_jittering:# 是否进行颜色攻击
        :param images_filter: #
        :param data_name: data数据的名字
        :param label_name: 标签的名字
        :param kwargs:
        """
        ...
        if path_imgrec:
            ...
            if header.flag>0:
              ...
              self.imgidx = [] # 保存
              self.id2range = {} # 记录，每个ID的范围，可以理解为其中有几张图片
              ...
              for identity in self.seq_identity:
                  ...
                  # a,b表示序列同一个ID的图像的起始标记到结束标记
                  a,b = int(header.label[0]), int(header.label[1])

                  count = b-a # 得到每个人图像的数目，如果小于images_filter则不参与训练
